...
<body class="bodyColor min-h-screen flex flex-col items-center bg-gray-900 text-white bg-cover bg-center p-4"
    ...
    <div class="rounded-lg shadow-md p-6 border-2 border-black max-w-3xl w-full" style="background-color: #e4e6d4;">
        <div class="flex flex-row items-center">
            ...
            <div class="mt-4">
                <img src="{{ asset('images/WebsiteImage1.png') }}"
                    class="w-full rounded border-2 border-black max-w-xs mx-auto alt='Pizza'">
            </div>
        </div>
    </div>
    <div class="rounded-lg shadow-md border-2 border-black p-6 mt-6 max-w-3xl w-full"
        style="background-color: #e4e6d4;">
    ...
    <div>
        @if ($order->status == 'pending')
            <form action="{{ route('orders.cancel', ['order' => $order->id]) }}" method="POST">
                @csrf
                @method('PUT')
                <button class="AnnulerenKnop" type="submit" onclick="return confirm('Weet je zeker dat je deze bestelling wilt annuleren?')">
